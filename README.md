...
`tests/` and `resources/` are the main directories to show my work.
...
| `tests/` | The Robot Framework suites. `vendomatic.robot` tests the library; `demo.robot` is a smoke test that only proves the tooling runs; `http.robot` tests the HTTP API. Run them with `robot tests/`. |
| `resources/` | The Robot resource directory. `vendomatic.resource` provides keywords for the Python object tests. `http.resource` provides keywords for the HTTP API tests. |
